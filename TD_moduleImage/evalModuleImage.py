...
if readme != "":
	...
	if MODE == "SEMIAUTO":
		...
		msg( read + " pas assez detaille", 0.5)
else:
...
print("===> assert ...")
search = ["assert"]
files = ["src/image.h", "src/Image.h" , "src/pixel.h" , "src/Pixel.h", "src/image.cpp", "src/Image.cpp" , "src/pixel.cpp" , "src/Pixel.cpp"]
n = 0
for f in files:
	for s in search:
		n += countInFile(f,s)
...
if MODE=="FULLAUTO":
	...
	for f in files:
		if isfile(f):
			...
			if texte.find("Image")==-1 or texte.find("getPix")==-1 or texte.find("setPix")==-1 or texte.find("dessinerRectangle")==-1 or texte.find("effacer")==-1:
	...
	points = int(input('nombre de points a deduire ? '))
	if points>0:
